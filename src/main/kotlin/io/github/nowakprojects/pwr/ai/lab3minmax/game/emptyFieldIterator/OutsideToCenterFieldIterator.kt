 ...
 */
class OutsideToCenterFieldIterator : EmptyFieldGenerator {
    ...
    override fun getIterator(stratego: Stratego) =
            getSpiral(stratego.bordMatrix.size)
                    .filter { stratego.bordMatrix[it.first][it.second] == Stratego.FieldType.EMPTY }.iterator()

    private fun getSpiral(size: Int) =
            if (CenterToOutsideFieldIterator.cacheMap.containsKey(size))
                cacheMap[size]
            else {
                cacheMap[size] = IteratorHelper.spiralPrint(size)
                cacheMap[size]
            }!!
